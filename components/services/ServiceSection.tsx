...
export default function ServiceSection({
  ...
  return (
    <section
      id={id}
      className={`service-section py-32 scroll-mt-[124px] ${
        reversed ? "bg-[#f8fafc]" : "bg-white"
      }`}
    >
      <div className="section-container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
                ...
                {features.map((feature, index) => (
                  ...
                  >
                    ...
                    <span className="text-[0.95rem] text-[#475569]">
                      <strong className="text-[#0f172a]">{feature.title}</strong>{" "}
                      {feature.description}
                    </span>
                  </li>
                ))}
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                {secondaryCta && (
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-8 py-[0.9rem] bg-transparent text-[#334155] font-semibold text-[0.95rem] border-[1.5px] border-[#e2e8f0] rounded-[10px] transition-all duration-250 hover:border-[#36a5ff] hover:text-[#006ec7] hover:bg-[#f0f7ff] hover:-translate-y-0.5"
                  >
                    {secondaryCta.text}
                  </Link>
                )}
          ...
          <FadeUp delay={200}>
            <div
              className={`relative rounded-[20px] overflow-hidden h-[480px] group ${
                reversed ? "lg:order-1" : ""
              }`}
            >
              <Image
